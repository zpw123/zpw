...
public class DBHelper {
	 ...
	 * @return 
	 */
	public int getIndex(String sql, List<Object> params){
		int result=0;
		...
		try {
			...
			while (rs.next()) {
				...
				doc.add(new Field("gid", rs.getString(1), Field.Store.YES,Field.Index.ANALYZED));
				doc.add(new Field("gname", rs.getString(2), Field.Store.YES,Field.Index.ANALYZED));
	...
	public void search(String name){
		...
		Directory dir;
		try {
			dir = new SimpleFSDirectory(new File(dirPathString));
			IndexReader reader = IndexReader.open(dir);
			IndexSearcher searcher = new IndexSearcher(reader);
			QueryParser parser = new QueryParser(Version.LUCENE_35, "gname", new StandardAnalyzer(Version.LUCENE_36));
			System.out.println(name);
			Query q = parser.parse(name);
			System.out.println(q);
			TopDocs tds = searcher.search(q, 4);
			ScoreDoc[] sds = tds.scoreDocs;
			for (ScoreDoc sd : sds) {
				System.out.println(sd.score);
				int docName = sd.doc;
				Document doc = searcher.doc(docName);
				String gname = doc.get("gname");
				String gid = doc.get("gid");
				System.out.println("Gname:"+gname+"   Gid:"+gid);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
